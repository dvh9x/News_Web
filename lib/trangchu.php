<?php 
	...
	function tinmoinhat_mottin($conn){
		$sql = "SELECT * FROM tin
		WHERE CURDATE() > Ngaybatdau 
				AND CURDATE() < Ngayketthuc
				ORDER BY idTin DESC
				LIMIT 0,1";
	...
	function TimKiem2($conn,$tukhoa,$from,$to){
		$sql = "SELECT * FROM tin
				WHERE TieuDe REGEXP '$tukhoa' AND Ngay >= '$from' AND Ngay <='$to'
				ORDER BY idTin DESC
				";
		return mysqli_query($conn,$sql);		
	}
